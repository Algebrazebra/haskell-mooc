...
appendAll ref strings = modifyIORef ref append
  where
    append x = x ++ (foldr (++) "" strings)
...
swapIORefs a b = do
  aVal <- readIORef a
  bVal <- readIORef b
  modifyIORef a (\_ -> bVal)
  modifyIORef b (\_ -> aVal)
...
hSelectLines h nums = do
  lines <- hFetchLines h
  return $ selectIndices nums lines

enumerateLines :: [a] -> [(Int, a)]
enumerateLines = zip [1 ..]

selectIndices :: [Int] -> [a] -> [a]
selectIndices indices xs = map snd $ filter isIndex (enumerateLines xs)
  where
    isIndex tuple = fst tuple `elem` indices
