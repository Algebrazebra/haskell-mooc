...
rangeOf xs = largestElement - smallestElement
  where
    smallestElement = foldr1 min xs
    largestElement = foldr1 max xs
...
longest :: Ord a => [[a]] -> [a]
longest xs = foldr maxByLength [] xs
  where
    maxByLength a b
...
incrementKey key [] = []
incrementKey key (pair : rest) =
  if (isMatch key pair)
    then incrementByOne pair : incrementKey key rest
    else pair : incrementKey key rest
  where
    isMatch k pair = fst pair == k
    incrementByOne (key, value) = (key, value + 1)
...
winner scores player1 player2
  | scorePlayer1 >= scorePlayer2 = player1
  | otherwise = player2
  where
    lookup player = Map.findWithDefault 0 player scores
    scorePlayer1 = lookup player1
    scorePlayer2 = lookup player2
...
freqs [] = Map.empty
freqs (x : xs) = Map.alter (incrementCount) x (freqs xs)
  where
    incrementCount Nothing = Just 1
    incrementCount (Just c) = Just (c + 1)
...
transfer from to amount bank
  ...
  | isNotEnoughMoney from amount bank = bank
  | amount < 0 = bank
  | otherwise = Map.adjust (addAmount) to (Map.adjust (subtractAmount) from bank)
  where
    isNotEnoughMoney from amount bank = case Map.lookup from bank of
      Just (balance) -> balance < amount
      Nothing -> True
    subtractAmount balance = balance - amount
    addAmount balance = balance + amount
...
swap idxA idxB array = array // [(idxA, valB), (idxB, valA)]
  where
    valA = array ! idxA
    valB = array ! idxB
...
maxIndex array = fst largestElement
  where
    largestElement = foldr1 maxTuple associations
    maxTuple a b = if (snd a >= snd b) then a else b
    associations = assocs array
