...
inputInOutput input output =
  let outputValues = map fst output
   in forAll (elements input) (\i -> i `elem` outputValues)
...
outputInInput input output = forAll (elements output) (\(elem, value) -> countElement elem input == value)
  where
...
frequenciesProp freq nonEmptyInput =
  let input = getNonEmpty nonEmptyInput
      output = freq input
...
genList :: Gen [Int]
genList = sort <$> do
  vectorLength <- choose (3, 5)
  let vectorElements = elements [0 .. 10]
  vectorOf vectorLength vectorElements
...
instance Arbitrary Arg where
  arbitrary = oneof [numbersFromRange, lettersFromRange] 
    where
      numbersFromRange = elements [Number i | i <- [0..10]]
      lettersFromRange = elements [Variable c | c <- "abcxyz"]

instance Arbitrary Expression where
  arbitrary = oneof [minusGen, plusGen]
    where 
      minusGen = do
        x <- arbitrary
        y <- arbitrary
