...
workload nExercises hoursPerExercise
  | totalTime > 100 = "Holy moly!"
  | totalTime < 10 = "Piece of cake!"
  | otherwise = "Ok."
  where
    totalTime = nExercises * hoursPerExercise
...
countValid str = length $ filter (\x -> x) (map isValid str)
  where
    isValid :: String -> Bool
    isValid xs = isEqualAtIndices 2 4 xs || isEqualAtIndices 3 5 xs
    isEqualAtIndices idx idy xs = xs !! idx == xs !! idy
...
repeated [] = Nothing
repeated (x : []) = Nothing
repeated (last : current : xs)
  | last == current = Just current
  | otherwise = repeated (current : xs)
...
sumSuccess xs = foldr sumEithers (Left "no data") xs
  where
    sumEithers :: Either String Int -> Either String Int -> Either String Int
    sumEithers (Right a) (Right b) = Right (a + b)
    sumEithers (Right a) (Left b) = Right a
    sumEithers (Left a) (Right b) = Right b
    sumEithers (Left a) (Left b) = Left b
...
open input (Open combination) = Open combination
open input (Closed combination) = if combination == input then (Open combination) else (Closed combination)
...
changeCode newCombination (Open _) = Open newCombination
changeCode newCombination closedLock = closedLock
...
instance Eq Text where
  (==) (Text a) (Text b) = (filter notWhiteSpace a) == (filter notWhiteSpace b)
...
compose _ [] = []
compose [] _ = []
compose ((k1, v1) : abs) bcs = case (lookup v1 bcs) of
  Just v2 -> (k1, v2) : (compose abs bcs)
