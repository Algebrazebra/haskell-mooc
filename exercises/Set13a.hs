...
split str = getFirstAndLast (words str)
  where
    getFirstAndLast :: [String] -> Maybe (String, String)
    getFirstAndLast (first : last : []) = Just (first, last)
    getFirstAndLast _ = Nothing
...
checkNumber (for, sur) = if all (not . isDigit) (for ++ sur) then Just (for, sur) else Nothing
...
checkCapitals (for, sur) = if all (isUpper . head) [for, sur] then Just (for, sur) else Nothing
...
countAndLog predicate [] = return 0
countAndLog predicate (x : xs)
  | predicate x = do
    msg (show x)
    res <- countAndLog predicate xs
    return $ res + 1
  | otherwise = countAndLog predicate xs
...
balance accountName = BankOp (\(Bank b) -> ((balance b), Bank b))
  where
    balance bank = Map.findWithDefault 0 accountName bank
...
paren c = do
  oldState <- get
  let changeState = case c of
        '(' -> \x -> x + 1
        ')' -> \x -> x - 1
        otherwise -> \x -> x
  if oldState < 0
    then put $ negate 1
    else put $ changeState oldState
...
count x = get >>= \xs -> put (countHelper x xs)
  where
    countHelper :: Eq a => a -> [(a, Int)] -> [(a, Int)]
    countHelper key xs
      | ifExists key xs = increment key xs
      | otherwise = append key xs
    increment key xs = fmap (\(k, v) -> if k == key then (k, v + 1) else (k, v)) xs
    append key xs = (key, 1) : xs
    ifExists key xs = case lookup key xs of
      Just _ -> True
      Nothing -> False
