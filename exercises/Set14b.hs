...
balance db account = do
    ...
    --For debug purposes: putStrLn (show rows)
    return
        $ sum          -- final sum of all rows
        $ map sum rows -- sums ints in row (there is only one int)
...
parseCommand [command, account, amount] | command == T.pack "deposit" = parseInt amount >>= \parsed -> Just (Deposit account parsed)
                                        | command == T.pack "withdraw" = parseInt amount >>= \parsed -> Just (Withdraw account parsed)
                                        | otherwise = Nothing
parseCommand [command, account] | command == T.pack "balance" = Just (Balance account)
                                | otherwise = Nothing
...
perform db (Just (Withdraw account amount)) = withdraw db account amount >> return (T.pack "OK")
perform db (Just (Balance account)) = fmap (T.pack . show) (balance db account)
...
simpleServer request respond = respond $ responseLBS status200 [] (convertStringToBytes "BANK")
  where
    convertStringToBytes str = LB.fromStrict (encodeUtf8 (T.pack str))
...
withdraw db account amount = execute db depositQuery (account, negate amount)
