...
buyOneMore (MkShoppingEntry product price count) = MkShoppingEntry product price (count + 1)
...
rgb (Invert color) = zipWith (-) [1.0, 1.0, 1.0] (rgb color)
...
toNat z = if z < 0
          then Nothing
          else Just (nonNegativeToNat z)

nonNegativeToNat :: Int -> Nat
nonNegativeToNat 0 = Zero
nonNegativeToNat n = PlusOne (nonNegativeToNat (n-1))
...
prettyPrint End = ""
prettyPrint (O remainingBits) = (prettyPrint remainingBits) ++ "0"
prettyPrint (I remainingBits) = (prettyPrint remainingBits) ++ "1"

fromBin :: Bin -> Int
fromBin bits = fromBin' bits 0

fromBin' :: Bin -> Int -> Int
fromBin' (O End) _ = 0
fromBin' (I End) i = 2^i
fromBin' (O xs)  i = fromBin' xs (i+1)
fromBin' (I xs)  i = fromBin' xs (i+1) + 2^i
