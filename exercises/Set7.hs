...
data State = Start | ContainsEggs | ContainsFlour | ContainsSugar | ContainsFlourAndSugar | IsMixed | Error | Finished
  deriving (Eq, Show)

step :: State -> Event -> State
step Finished _ = Finished
step Start AddEggs = ContainsEggs
step Start _ = Error
step ContainsEggs AddFlour = ContainsFlour
step ContainsEggs AddSugar = ContainsSugar
step ContainsEggs _ = Error
step ContainsFlour AddSugar = ContainsFlourAndSugar
step ContainsFlour _ = Error
step ContainsSugar AddFlour = ContainsFlourAndSugar
step ContainsSugar _ = Error
step ContainsFlourAndSugar Mix = IsMixed
step ContainsFlourAndSugar _ = Error
...
reverseNonEmpty (x :| []) = x :| []
reverseNonEmpty (x :| xs) = let reversed = reverse (x:xs) in (head reversed) :| (tail reversed)
...
instance (Eq a, Ord a) => Semigroup (Set a) where
  (<>) (Set []) (Set b) = Set b
  (<>) (Set a) (Set []) = Set a
  (<>) (Set a) (Set b) = add (head b) (Set a) <> (Set (tail b))
...
passwordAllowed password (ContainsSome str) = any (\x -> x `elem` str) password
passwordAllowed password (DoesNotContain str) = all (\x -> not (x `elem` str)) password
...
data Arithmetic = Value Integer | Operation String Arithmetic Arithmetic
  deriving (Show)
...
render (Value x) = show x
render (Operation "+" left right) = "(" ++ (render left) ++ "+" ++ (render right) ++ ")"
render (Operation "*" left right) = "(" ++ (render left) ++ "*" ++ (render right) ++ ")"
