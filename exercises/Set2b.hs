...
leftpad str target
  | length str >= target = str
  | length str < target = leftpad (" " ++ str) target
...
countdown n = countdown' (n) (readyStr) ++ liftOffStr
  where
    readyStr = "Ready! "
    liftOffStr = "Liftoff!"
...
countdown' 0 acc = acc
countdown' n acc = countdown' (n - 1) (acc ++ countWithDots)
  where
    countWithDots = show n ++ "... "
...
smallestDivisor' n k
  | n `mod` k == 0 = k
  | n `mod` k /= 0 = smallestDivisor' n (k + 1)
