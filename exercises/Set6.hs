...
instance Eq RationalNumber where
  (RationalNumber num denom) == (RationalNumber num2 denom2) = num * denom2 == num2 * denom
...
simplify (RationalNumber numerator denominator) = RationalNumber simplerNumerator simplerDenominator
  where
    simplerNumerator = numerator `div` greatestCommonFactor
    simplerDenominator = denominator `div` greatestCommonFactor
...
instance Num RationalNumber where
  ...
  abs number@(RationalNumber n d) = if n < 0 then negate number else number
