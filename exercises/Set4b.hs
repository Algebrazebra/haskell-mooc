...
countHelper elem acc = case elem of
  Just (_) -> acc
  Nothing -> acc + 1
...
slHelper elem (acc, len) = (acc + elem, len + 1)
...
largestHelper elem [] = [elem]
largestHelper elem acc@(largest : _)
  | elem == largest = elem : acc
  | elem > largest = [elem]
  | elem < largest = acc
...
headHelper elem acc = case acc of
  Nothing -> Just elem
  Just _ -> Just elem
...
lastHelper elem acc = case acc of
  Nothing -> Just elem
  Just last -> Just last