...
instance Functor TwoList where
  ...
  fmap f (TwoNode first second tail) = TwoNode (f first) (f second) (fmap f tail)
...
count elem foldable = foldr (\a b -> if a == elem then b + 1 else b) 0 foldable
...
instance Foldable TwoList where
  ...
  foldr f acc (TwoNode x y t) = f x $ f y $ foldr f acc t
...
sumTree :: Monoid m => Tree m -> m
sumTree Leaf = mempty
sumTree (Node value left right) = mappend (sumTree left) (mappend value (sumTree right))
