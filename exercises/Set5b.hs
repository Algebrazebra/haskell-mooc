...
treeMax Empty = 0
treeMax (Node value left right) = max maxLeft maxRight
  where
    maxLeft = max value (treeMax left)
    maxRight = max value (treeMax right)
...
allValues condition (Node value left right) = condition value && conditionLeft && conditionRight
  where
    conditionLeft = allValues condition left
    conditionRight = allValues condition right
...
mapTree f (Node value left right) = Node (f value) mapLeft mapRight
  where
    mapLeft = mapTree f left
...
cull target Empty = Empty
cull target (Node value left right)
  | target == value = Empty
  | otherwise = Node value (cull target left) (cull target right)
...
walk (step : remainingSteps) (Node value leftTree rightTree) = case step of
  StepL -> walk remainingSteps leftTree
  StepR -> walk remainingSteps rightTree
...
set (StepL : path) value (Node val left right) = Node val (set path value left) right
set (StepR : path) value (Node val left right) = Node val left (set path value right)
...
search value (Node val left right)
  | val == value = Just []
  | val /= value = (fmap (StepL :) (search value left)) `orElse` (fmap (StepR :) (search value right))

orElse :: Maybe a -> Maybe a -> Maybe a
orElse (Just value) _ = (Just value)
orElse _ (Just value) = (Just value)
orElse _ _ = Nothing
