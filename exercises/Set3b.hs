...
buildList start count end = loop start count [end]
  where
    loop :: Int -> Int -> [Int] -> [Int]
    loop start 0 acc = acc
    loop start count acc = loop start (count - 1) (start : acc)
...
sums' idx stop
  | idx == stop = []
  | otherwise = addTillN (idx + 1) : sums' (idx + 1) stop

addTillN :: Int -> Int
addTillN 1 = 1
addTillN n = n + addTillN (n - 1)
...
mylast :: a -> [a] -> a
mylast def [] = def
mylast _ [value] = value
mylast def (head : tail) = mylast def tail
...
sorted :: [Int] -> Bool
sorted [] = True
sorted [onlyValue] = True
sorted (first : second : rest) = first <= second && sorted (second : rest)
...
sumsOf' :: Int -> [Int] -> [Int]
sumsOf' acc [] = []
sumsOf' acc (first : rest) = newAcc : sumsOf' newAcc rest
  where
    newAcc = first + acc
...
merge xs [] = xs
merge [] ys = ys
merge xs@(x : tailX) ys@(y : tailY)
  | x > y = y : (merge xs tailY)
  | x < y = x : (merge tailX ys)
  | otherwise = x : y : merge tailX tailY
...
mymaximum bigger initial (first : rest)
  | bigger initial first = mymaximum bigger initial rest
  | bigger first initial = mymaximum bigger first rest
  | otherwise = mymaximum bigger first rest
