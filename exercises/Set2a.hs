...
updateAt :: Int -> a -> [a] -> [a]
updateAt i x xs = firstPart ++ [x] ++ lastPart
  where
    firstPart = take i xs
    lastPart = drop (i + 1) xs
...
isPalindrome [] = True
isPalindrome [_] = True
isPalindrome [first, last] = first == last
isPalindrome str = (head str == last str) && isPalindrome betweenFirstAndLast
  where
    betweenFirstAndLast = chopOffFirstAndLast str

chopOffFirstAndLast :: String -> String
chopOffFirstAndLast str
  | length str < 3 = ""
  | otherwise = tail (take (length str - 1) str)
...
palindromify s
  | isPalindrome s = s
  | otherwise = palindromify (chopOffFirstAndLast s)
